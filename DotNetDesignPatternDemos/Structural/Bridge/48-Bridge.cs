using System;
using System.Collections.Generic;
using System.Text;
using Autofac;

namespace DotNetDesignPatternDemos.Structural.Bridge
{
    public interface IRenderer48
    {
        void RenderCircle(float radius);
    }

    public class VectorRenderer48 : IRenderer48
    {
        public void RenderCircle(float radius)
        {
            Console.WriteLine($"Drawing a circle of radius {radius}");
        }
    }

    public class RasterRenderer48 : IRenderer48
    {
        public void RenderCircle(float radius)
        {
            Console.WriteLine($"Drawing pixels for circle of radius {radius}");
        }
    }

    public abstract class Shape48
    {
        protected IRenderer48 renderer;

        // a bridge between the shape that's being drawn an
        // the component which actually draws it
        public Shape48(IRenderer48 renderer)
        {
            this.renderer = renderer;
        }

        public abstract void Draw();
        public abstract void Resize(float factor);
    }

    public class Circle : Shape48
    {
        private float radius;

        public Circle(IRenderer48 renderer, float radius) : base(renderer)
        {
            this.radius = radius;
        }

        public override void Draw()
        {
            renderer.RenderCircle(radius);
        }

        public override void Resize(float factor)
        {
            radius *= factor;
        }
    }

    public class Bridge
    {
        public static void Start()
        {
            //var raster = new RasterRenderer();
            //var vector = new VectorRenderer();
            //var circle = new Circle(vector, 5, 5, 5);
            //circle.Draw();
            //circle.Resize(2);
            //circle.Draw();

            var cb = new ContainerBuilder();
            cb.RegisterType<VectorRenderer48>().As<IRenderer48>();
            cb.Register((c, p) => new Circle(c.Resolve<IRenderer48>(),
              p.Positional<float>(0)));
            using (var c = cb.Build())
            {
                var circle = c.Resolve<Circle>(
                  new PositionalParameter(0, 5.0f)
                );
                circle.Draw();
                circle.Resize(2);
                circle.Draw();
            }
        }
    }
}
